...
Fixed Zoom chooses an interpretation after the pointer leaves an 18-pixel radius around the original press: at least 60% vertical intent selects zoom, while at least 60% horizontal intent selects pan. Once selected, the interpretation stays stable until its active coordinate returns within 22 pixels of the press point: Pan can swap near the original X coordinate, while Zoom can swap near the original Y coordinate. The current interpretation remains active and moving throughout this crossover zone until the opposite direction qualifies. One black square follows the pointer. The red dot starts at its centre and follows cursor displacement one-to-one in both axes while it remains inside the untextured square, without affecting the timeline. Touching the active-axis wall engages Pan or Zoom, reveals the ridged dial texture, and rapidly narrows the square into the corresponding horizontal or vertical dial. Returning inside the wall disengages timeline movement, removes the dial texture, and resets both marker axes at the re-entry point. All previous local offset is discarded; subsequent main-axis and perpendicular movement are tracked from this new dead-zone centre. As the shape narrows, its perpendicular range progressively converges on the active dial's centreline. Once the dot turns back toward the centre outside the square zone, perpendicular tracking remains centred until the square is restored. The dot fades according to the pointer's actual distance from the original press coordinate. Vertical control stops active panning and rescales the segments around the centred Head.

The complete state model, local-origin behavior, retraction rules, visual geometry, and implementation invariants are documented in [The Pan and Zoom Gesture Dial](gesture-dial.md).
