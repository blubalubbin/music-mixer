...
Fixed Zoom chooses an interpretation after the pointer leaves an 18-pixel radius around the original press: at least 60% vertical intent selects zoom, while at least 60% horizontal intent selects pan. Once selected, the interpretation stays stable until its active coordinate returns within 22 pixels of the press point: Pan can swap near the original X coordinate, while Zoom can swap near the original Y coordinate. The current interpretation remains active and moving throughout this crossover zone until the opposite direction qualifies. A crossed pair of ridged black dials follows the pointer. Both remain visible, but only the active dial rolls and sits above the frozen inactive dial. One shared red dot travels through their central intersection, continuing smoothly onto the newly active axis rather than handing off between separate markers. The dot fades according to the pointer's actual distance from the original press coordinate and deflects slightly within the active dial's thickness in response to perpendicular movement. Vertical control stops active panning and rescales the segments around the centred Head.
